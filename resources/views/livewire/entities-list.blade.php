                        ...
                        <table class="min-w-full border divide-y divide-gray-200">
                            ...
                            <tbody wire:sortable="updateOrder" class="bg-white divide-y divide-gray-200 divide-solid">
                                @foreach ($entities as $entity)
                                    <tr class="bg-white" wire:sortable.item="{{ $entity->id }}"
                                        wire:key="{{ $loop->index }}">
                                        <td class="px-6">
                                        ...
                                        {{-- Inline Edit Start --}}
                                        <td
                                            class="@if ($editedEntityId !== $entity->id) hidden @endif px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                                            <input wire:model="entity.name" id="entity.name"
                                        ...
                                        <td
                                            class="@if ($editedEntityId !== $entity->id) hidden @endif px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                                            <input wire:model="entity.slug" id="entity.slug"
                                                class="py-2 pr-4 pl-2 mt-2 w-full text-sm rounded-lg border border-gray-400 sm:text-base focus:outline-none focus:border-blue-400" />
                                            @error('entity.slug')
                                                <span class="text-sm text-red-500">{{ $message }}</span>
                                            @enderror
                                        ...
                                        {{-- Show Entity Name/Slug End --}}

                                        <td class="px-6">
                                            <div
                                                class="inline-block relative mr-2 w-10 align-middle transition duration-200 ease-in select-none">
                                                <input wire:model="active.{{ $entity->id }}"
                                                    wire:click="toggleIsActive({{ $entity->id }})" type="checkbox"
                                                    name="toggle" id="{{ $loop->index . $entity->id }}"
                                                ...
                                                <label for="{{ $loop->index . $entity->id }}"
                                        ...
                                        <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                                            @if ($editedEntityId === $entity->id)
                                                ...
                                                <x-button wire:click="cancelEntityEdit">
                                                    Cancel
                                                </x-button>
                                            @else
                                                <x-button wire:click="editEntity({{ $entity->id }})">
                                                    Edit
                                                </x-button>
                                                <button wire:click="deleteConfirm('delete', {{ $entity->id }})"
            ...
            <form wire:submit.prevent="save" class="w-full">
                <div class="flex flex-col items-start p-4">
                    <div class="flex items-center pb-4 mb-4 w-full border-b">
                        <div class="text-lg font-medium text-gray-900">Create Entity</div>
                    ...
                    <div class="mb-2 w-full">
                        <label class="block text-sm font-medium text-gray-700" for="entity.name">
                            Name
                        </label>
                        <input wire:model="entity.name" id="entity.name"
                            class="py-2 pr-4 pl-2 mt-2 w-full text-sm rounded-lg border border-gray-400 sm:text-base focus:outline-none focus:border-blue-400" />
                    ...
                    <div class="mb-2 w-full">
                        <label class="block text-sm font-medium text-gray-700" for="entity.slug">
                            Slug
                        </label>
                        <input wire:model="entity.slug" id="entity.slug"
                            class="py-2 pr-4 pl-2 mt-2 w-full text-sm rounded-lg border border-gray-400 sm:text-base focus:outline-none focus:border-blue-400" />
